    ...
    public async Task ChatCompletionAgentAsync()
    {
        ...
        AgentGroupChat chat = new(traffLawAgent, workerLawAgent)
        {
            ExecutionSettings = new()
            {
                ...
                {
                    ResultParser = (result) => !string.IsNullOrEmpty(result.GetValue<string>()),
                    MaximumIterations = 5,
                    AutomaticReset = true
                },
        ...
        await InvokeAgentAsync("紅燈只要沒有車是可以右轉的");
        await InvokeAgentAsync("上下班刷卡只是應付勞基法，不是真的要記錄工時");
